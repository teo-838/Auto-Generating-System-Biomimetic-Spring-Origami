...
#Main function
def AUTO_GENERATING_SYSTEM():
    excelorderdata=read_csv(Excelfilename)#extract excel data
    excelorderdata= excelorderdata[2:] #Remove the first two row: The title "Auto-Generating System" and the row of "Parameter Label"
    ...
    f1 = plt.figure("Auto-Generating System")#setup the figure and name as "Auto-Generating System"
    ...
    lstoffacet=[]
    lstofangle=[]
    lstofforallopacity=[]#for <Iterative> mode: list of Opacity for <Iterative> mode.
    lstofsptrans=[]#for <Specific Structure> mode: list of Transparency crease index for <Specific Structure> mode
    lstofspopaci=[]#for <Specific Structure> mode: list of Opacity crease index and opacity level (in x %) for <Specific Structure> mode
    ...
    #Special row for <Specific Structure> mode: Opacity and transparency data
    dicopacity={}
    for jj in range(2,len(excelorderdata[11])):# jj is the number of block
        lstofspopaci=[]
        for j in excelorderdata[11::]:  ## j is the row contain crease opacity for "Specific Structure" function.
            if j[jj]!='':
                lstofspopaci.append(j[jj])
        dicopacity[jj]=lstofspopaci
    ##print(lstofspopaci)
    ##print(dicopacity)
    #
    ...
    for ll in range(total_number_of_block):
        ...
        line_width_raw_data=excelorderdata[0][ll+2]
        ...
        if choice_ratio_or_angle=='Angle':
        ...
        if choice_ratio_or_angle=='Ratio':
        ...
        ##Process via Specific Structure of Mode: <Normal>=normal mode, <Iterative>=iterative crease opacity mode, <Specific Structure>=specific crease opacity mode
        #if <Iterative> mode be selected
        if current_block_choice_ratio_or_angle=='Iterative':
            current_block_opcaity_for_All_mode=lstofforallopacity.pop(0)#store the opacity which need to apply in <Iterative> mode
            #if opacity of <Iterative> mode is 0, i.e. fully transparency, call ALL_MODE_TRANSPARENCY_GENERATOR
            if current_block_opcaity_for_All_mode=='0':
                ALL_MODE_TRANSPARENCY_GENERATOR(need_twist)
            #
            #if opacity of <Iterative> mode is not 0, i.e. partially transparency, call ALL_MODE_OPACITY_GENERATOR with opacity info lsts.
            else:
                ALL_MODE_OPACITY_GENERATOR(current_block_opcaity_for_All_mode,need_twist,line_width)
            #
        #
        
        #if <Normal> mode be selected
        if current_block_choice_ratio_or_angle=='Normal':
            GENERAL_MARROW_GENERATOR([],[],need_twist)#Use GENERAL_MARROW_GENERATOR with empty transparency and opacity lst
        #

        #if <Specific Structure> mode be selected
        if current_block_choice_ratio_or_angle=='Specific Structure':
    ...
    filename=outputfilename+ '.svg' # 'MARROW' +'_'+ current_block_total_number_of_row + '_' + current_block_total_number_of_column +'_'+ file_name_id+ '.svg' File name format: 'MARROW_[current_block_total_number_of_row]_[current_block_total_number_of_column]_[file_name_id].svg'
...
#RUN HERE ONWARDS
filenames=input('Excel File Name: ')
outputfilename = input('Output File Name: ')
Excelfilename=filenames+'.csv'
    
AUTO_GENERATING_SYSTEM()#Call function


#TeoYX-21122021 version
